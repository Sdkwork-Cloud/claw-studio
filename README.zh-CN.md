...
Claw Studio 是面向现代 Claw Studio 应用、共享 Web Shell 与 Tauri 桌面运行时的分包工作区。当前实现以 `upgrade/claw-studio-v3` 为功能基线，但已经重组为可维护的业务分包结构，并通过根级导入和架构校验约束长期演进。
...
- `@sdkwork/claw-studio-web`：可运行的 Web 应用与开发服务器
- `@sdkwork/claw-studio-desktop`：Tauri 桌面入口与原生桥接
- `@sdkwork/claw-studio-shell`：路由、布局、Provider、侧边栏、命令面板
- `@sdkwork/claw-studio-business`：共享 store 与跨业务编排
- `@sdkwork/claw-studio-domain`：纯类型与共享领域模型
- `@sdkwork/claw-studio-infrastructure`：环境配置、HTTP、i18n 与平台适配
- `@sdkwork/claw-studio-*`：垂直业务包，例如 `chat`、`market`、`settings`、`account`、`extensions`

仓库禁止跨包子路径导入。请使用 `@sdkwork/claw-studio-market` 这样的包根导入方式，而不是 `@sdkwork/claw-studio-market/src/...`。
...
默认 Web 开发服务器由 `packages/claw-studio-web/server.ts` 启动，地址为 `http://localhost:3001`。
...
pnpm check:parity  # 校验关键功能与 v3 基线一致
...
桌面端示例可参考 [`packages/claw-studio-desktop/.env.example`](./packages/claw-studio-desktop/.env.example)。
...
仓库内也保留了 `@sdkwork/claw-studio-docs` 这个应用内文档功能包。`docs/` 下的 VitePress 站点则是面向 GitHub 和开源协作者的公共项目文档。
