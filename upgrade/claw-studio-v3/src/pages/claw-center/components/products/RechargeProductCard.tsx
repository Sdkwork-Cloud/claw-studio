...
import { RechargeProduct } from '../../../../services/clawService';
...
export const RechargeProductCard = ({ product, onRequest }: { product: RechargeProduct, onRequest: (name: string) => void }) => {
  ...
  return (
    <ProductCardWrapper product={product} onRequest={onRequest}>
      <div className="flex items-center gap-1.5 col-span-2"><Zap className="w-3.5 h-3.5" /> {t('products.labels.provider', 'Provider')}: <span className="font-medium text-zinc-900 dark:text-zinc-100">{product.provider}</span></div>
