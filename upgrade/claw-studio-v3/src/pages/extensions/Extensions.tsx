...
import { Search, Puzzle, Download, Star, ShieldCheck, ChevronRight, Settings, Trash2, CheckCircle2 } from 'lucide-react';
...
export function Extensions() {
  ...
  const filteredExtensions = extensions.filter(ext => {
    ...
    const matchesTab = activeTab === 'discover' ? true : ext.installed;
              ...
              {filteredExtensions.map(ext => (
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    <img src={ext.icon} alt={ext.name} className="w-14 h-14 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 object-cover" referrerPolicy="no-referrer" />
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 truncate">{ext.name}</h3>
                      <p className="text-sm text-zinc-500 dark:text-zinc-400 truncate">{ext.author}</p>
                    </div>
                  </div>
