...
import { useAppStore, ThemeMode, ThemeColor, Language } from '../../store/useAppStore';
...
export function GeneralSettings() {
  ...
  useEffect(() => {
    settingsService.getPreferences().then(p => setPrefs(p.general));
  }, []);
  ...
  const handleToggle = async (key: keyof UserPreferences['general']) => {
    ...
    try {
      await settingsService.updatePreferences({ general: { ...prefs, [key]: newValue } });
    } catch (error) {
      setPrefs({ ...prefs, [key]: !newValue });
