...
import { Sparkles, Plus, Trash2, Edit2, Save, X, Check, Settings2, Key, Globe, Cpu, Hash, MessageSquare, Star, ChevronDown, AlertCircle } from 'lucide-react';
import { useLLMStore, LLMChannel, LLMModel } from '../../store/useLLMStore';
import { motion, AnimatePresence } from 'motion/react';
import { useInstanceStore } from '../../store/useInstanceStore';

export function LLMSettings() {
  const { activeInstanceId } = useInstanceStore();
  const { channels, addChannel, updateChannel, removeChannel, updateConfig, setActiveChannel, setActiveModel, getInstanceConfig } = useLLMStore();
  ...
  if (!activeInstanceId) {
    return (
      <div className="p-6 md:p-10 max-w-[1400px] mx-auto flex flex-col items-center justify-center h-64 text-center">
        ...
        <p className="text-zinc-500 dark:text-zinc-400">Please select an instance from the sidebar to configure LLM settings.</p>
      </div>
    );
  }
    ...
    <div className="space-y-10 max-w-5xl mx-auto pb-12">
      ...
      <section>
        <div className="flex items-center gap-2 mb-6 px-2">
          ...
          <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Instance Defaults</h3>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setActiveChannel(activeInstanceId, newChannelId);
                    const newChannel = channels.find(c => c.id === newChannelId);
                    if (newChannel && newChannel.models.length > 0) {
                      setActiveModel(activeInstanceId, newChannel.defaultModelId || newChannel.models[0].id);
                    }
                  }}
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                <select
                  value={activeModelId}
                  onChange={(e) => setActiveModel(activeInstanceId, e.target.value)}
                  className="w-full appearance-none px-4 py-3.5 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 transition-all text-zinc-900 dark:text-zinc-100 pr-10 cursor-pointer"
                >
      ...
      <section>
        <div className="flex items-center gap-2 mb-6 px-2">
          ...
          <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Instance Parameters</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow group">
            ...
            <input
              ...
              onChange={(e) => updateConfig(activeInstanceId, { temperature: parseFloat(e.target.value) })}
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow group">
            ...
            <input
              ...
              onChange={(e) => updateConfig(activeInstanceId, { maxTokens: parseInt(e.target.value) })}
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow group">
            ...
            <input
              ...
              step="0.05"
              value={config.topP}
              onChange={(e) => updateConfig(activeInstanceId, { topP: parseFloat(e.target.value) })}
              className="w-full accent-primary-500 mb-3"
            />
            <div className="flex justify-between text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
              <span>Focused</span>
