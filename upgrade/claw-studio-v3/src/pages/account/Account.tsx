...
import { Wallet, ArrowUpRight, ArrowDownRight, Plus, Download, CreditCard, History, TrendingUp, DollarSign, Clock, CheckCircle2, X } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { accountService, Transaction, AccountSummary } from '../../services/accountService';
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden flex flex-col min-h-[400px]">
          <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg">
              <button 
                ...
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'all' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
              >
                {t('account.all', 'All')}
              </button>
              <button 
                onClick={() => setActiveTab('income')}
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'income' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
              >
                {t('account.income', 'Income')}
              </button>
              <button 
                onClick={() => setActiveTab('expense')}
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'expense' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
              >
                {t('account.expense', 'Expense')}
              </button>
            </div>
          </div>
          ...
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800/50 flex-1">
            {isLoading ? (
              ...
              transactions.map(tx => (
                <div key={tx.id} className="p-4 sm:p-6 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                      tx.type === 'income' || tx.type === 'recharge' 
                    ...
                    <div>
                      <p className="font-semibold text-zinc-900 dark:text-zinc-100">{tx.desc}</p>
                  ...
                  <div className={`text-right font-bold whitespace-nowrap ${
                    tx.type === 'income' || tx.type === 'recharge' ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-900 dark:text-white'
                  }`}>
                    {tx.type === 'income' || tx.type === 'recharge' ? '+' : '-'}${tx.amount.toFixed(2)}
                  </div>
                </div>
              ))
            ) : (
