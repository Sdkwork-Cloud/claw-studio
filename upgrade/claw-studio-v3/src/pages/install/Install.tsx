...
export function Install() {
  ...
  const handleInstall = async () => {
    ...
    try {
      const result = await invoke('execute_install_script', { command });
      setInstallOutput(prev => prev + '\n' + result);
      setInstallStatus('success');
    } catch (err: any) {
      setInstallStatus('error');
      setInstallOutput(prev => prev + '\nError: ' + (err.message || String(err)) + '\n\n(Note: Tauri API is not available in this web preview environment. In a real Tauri app, this would execute the native installation script.)');
    }
  };
