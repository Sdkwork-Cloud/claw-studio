...
const CodeBlock = ({ code, language = 'bash', executable = false }: { code: string, language?: string, executable?: boolean }) => {
  ...
  const handleRun = async () => {
    ...
    try {
      // In a real Tauri app, this would call the Rust backend command
      const result = await invoke('execute_install_script', { command: code });
      setOutput(prev => prev + '\n' + result);
      setStatus('success');
    } catch (e: any) {
      setStatus('error');
      setOutput(prev => prev + '\nError: ' + (e.message || String(e)) + '\n\n(Note: Tauri API is not available in this web preview environment. In a real Tauri app, this would execute the command natively.)');
    }
  };
