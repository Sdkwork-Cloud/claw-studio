...
interface LLMState {
  channels: LLMChannel[];
  instanceConfigs: Record<string, InstanceLLMState>; // Keyed by instanceId
  ...
  persist(
    (set, get) => ({
      ...
      removeChannel: (id) => set((state) => {
        ...
        // Update instance configs if they were using the removed channel
        ...
        for (const instanceId in newInstanceConfigs) {
          if (newInstanceConfigs[instanceId].activeChannelId === id) {
             newInstanceConfigs[instanceId] = {
               ...
               activeModelId: newChannels.length > 0 ? (newChannels[0].defaultModelId || newChannels[0].models[0]?.id || '') : ''
             };
          }
        }
      ...
      setActiveChannel: (instanceId, channelId) => set((state) => ({
        instanceConfigs: {
      ...
      setActiveModel: (instanceId, modelId) => set((state) => ({
        instanceConfigs: {
          ...state.instanceConfigs,
          [instanceId]: {
            ...(state.instanceConfigs[instanceId] || DEFAULT_INSTANCE_CONFIG),
      ...
      updateConfig: (instanceId, configUpdate) => set((state) => {
