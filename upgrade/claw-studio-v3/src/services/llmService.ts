...
class LLMService implements ILLMService {
  ...
  async generateContent(options: AIRequestOptions): Promise<string> {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: fullPrompt,
      config: {
        systemInstruction: options.systemInstruction || 'You are an expert AI assistant helping a user write and edit content. Provide helpful, accurate, and concise responses.',
      }
    });
