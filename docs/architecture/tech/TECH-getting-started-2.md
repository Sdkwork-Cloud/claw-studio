> Migrated from `docs/zh-CN/guide/getting-started.md` on 2026-06-24.
...
这会启动 `@sdkwork/agentstudio-pc-web` 的 Vite 开发服务器，默认地址为 `http://localhost:3001`。
...
- `pnpm build:server` 用于构建原生 Server 二进制
- `pnpm build:desktop` 用于构建桌面端安装包
...
桌面端补充示例见 `packages/sdkwork-agentstudio-pc-desktop/.env.example`，服务端默认变量见 `packages/sdkwork-agentstudio-pc-server/.env.example`。
