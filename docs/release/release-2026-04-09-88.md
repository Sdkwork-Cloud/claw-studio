...
- Implemented the narrow repair:
  - added `SectionHeading(...)` and `SectionAvailabilityNotice(...)` back to `packages/sdkwork-claw-instances/src/components/InstanceWorkbenchPrimitives.tsx`
  - rewired `packages/sdkwork-claw-instances/src/pages/InstanceDetail.tsx` to import those primitives
