...
- Step 03 advanced `CP03-4` and preserved desktop startup-evidence descriptor state-store driver/profile through the shared kernel/runtime contract.
- `Kernel Center` now shows persisted startup descriptor state-store driver and profile ID in addition to the previously published startup summary fields.
...
- The loop repaired the next missing `CP03-4` shell-facing evidence link:
  - the raw desktop startup-evidence document already contained descriptor `stateStoreDriver` and `stateStoreProfileId`
  - the desktop kernel publication summary dropped those two descriptor storage identity fields before shell consumption
  - `Kernel Center` therefore lost which startup state-store driver and profile the snapshot represented, even though the producer already persisted those facts
- Implemented the narrow repairs:
  - added `descriptorStateStoreDriver` and `descriptorStateStoreProfileId` to the desktop kernel startup-evidence summary contract
...
- `docs/review/step-03-kernel-center-startup-evidence-descriptor-state-store-convergence-2026-04-08.md`
- `docs/架构/130-2026-04-08-desktop-startup-evidence-descriptor-state-store-owner.md`
- `docs/review/step-03-执行卡-2026-04-07.md`
- `docs/release/release-2026-04-08-45.md`
...
- The shell now depends on the published startup descriptor state-store fields remaining aligned with the raw desktop startup-evidence producer.
