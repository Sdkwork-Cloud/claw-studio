...
- Step 03 continued from the remaining `sdkwork-claw-instances` IDE dependency blocker into the next shell and tasks package-root contract layer.
...
- The loop repaired three linked contract drifts:
  - `sdkwork-claw-instances` had dropped its Monaco runtime ownership from `package.json` even though the instances workbench still lazily imports Monaco
  - `sdkwork-claw-settings` and `sdkwork-claw-instances` package roots were no longer publishing the exact file-bearing exports frozen by shell contract scripts
  - `sdkwork-claw-tasks` package root no longer explicitly exposed `GlobalTaskManager`, `taskService`, and `useTaskStore`
- Implemented the narrow repairs:
  ...
  - changed `packages/sdkwork-claw-settings/src/index.ts` to use explicit `KernelCenter.ts` and `ProviderConfigCenter.ts` root exports
  - changed `packages/sdkwork-claw-instances/src/index.ts` to explicitly export `./pages/Nodes.tsx`
  - changed `packages/sdkwork-claw-tasks/src/index.ts` to explicitly export `./components/GlobalTaskManager`, `./services/taskService`, and `./store/useTaskStore`
- Fresh verification:
