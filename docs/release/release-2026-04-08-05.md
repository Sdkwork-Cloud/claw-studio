...
- `packages/sdkwork-claw-install/src/services/openClawBootstrapService.test.ts` now records studio create/update calls with the real `StudioPlatformAPI` input types instead of `Record<string, unknown>` casts.
