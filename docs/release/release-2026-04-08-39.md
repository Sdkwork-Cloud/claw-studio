...
- Step 03 advanced `CP03-4` and preserved desktop startup-evidence diagnostic identity through the shared kernel/runtime contract.
- `Kernel Center` now shows persisted startup-evidence `runId` and `errorCause` in addition to the previously published summary fields.
...
- The loop repaired the next missing `CP03-4` shell-facing evidence link:
  - the raw desktop startup-evidence document already contained `runId` and `error.cause`
  - the desktop kernel publication summary dropped both fields before shell consumption
  - `Kernel Center` therefore lost the startup-attempt identity and causal hint even though the producer already persisted them
- Implemented the narrow repairs:
  - added `runId` and `errorCause` to the desktop kernel startup-evidence summary contract
...
- `docs/review/step-03-kernel-center-startup-evidence-diagnostic-identity-convergence-2026-04-08.md`
- `docs/架构/124-2026-04-08-desktop-startup-evidence-diagnostic-identity-owner.md`
- `docs/review/step-03-执行卡-2026-04-07.md`
- `docs/release/release-2026-04-08-39.md`
...
- The shell now depends on the published `runId/errorCause` pair remaining aligned with the raw desktop startup-evidence producer.
