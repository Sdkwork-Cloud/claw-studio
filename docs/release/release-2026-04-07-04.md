...
- Synchronized `@sdkwork/app-sdk` with the current local source-of-truth workspace and advanced the release pin to commit `e7bc761ce45acf142721b96f732b361c77a71e73`.
- Extended shared SDK release-source preparation and parity verification so GitHub release automation now validates `@sdkwork/app-sdk`, `@sdkwork/sdk-common`, and `@sdkwork/core-pc-react` together.
...
- Confirm `pnpm lint`, `pnpm check:desktop`, `pnpm check:server`, `pnpm build`, `pnpm server:build`, and `pnpm docs:build` succeed in release mode with the GitHub-backed shared SDK sources.
