...
- Continued the real `Step 07` service hotspot decomposition by moving managed OpenClaw workbench config shaping into a dedicated helper module.
- Realigned the `sdkwork-instances` contract suite and service barrel to the new helper-owned boundary for managed config snapshot construction, insights derivation, and workbench config cloning.
- Reduced the recorded `packages/sdkwork-claw-instances/src/services/instanceWorkbenchServiceCore.ts` hotspot from the prior `2635`-line baseline to a fresh `2462` lines while keeping OpenClaw authority unchanged.
...
- Rewired the workbench core so empty managed config snapshot construction, managed config insights derivation, managed config section counting, and managed config deep-clone shaping no longer live inline in:
  - `packages/sdkwork-claw-instances/src/services/instanceWorkbenchServiceCore.ts`
- Updated the service barrel so the new managed-config helper boundary stays consumable from the package root in:
...
- `docs/review/step-07-instance-detail分区一致性-2026-04-08.md`
- `docs/架构/134-2026-04-08-instance-detail-section-decomposition-progress.md`
- `docs/release/release-2026-04-08-72.md`
...
- `Step 07` is still not closable. `CP07-3` remains open because `InstanceDetail.tsx` is still a `2553`-line page hotspot and `instanceWorkbenchServiceCore.ts` still remains above closure size at `2462` lines.
