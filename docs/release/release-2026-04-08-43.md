...
- Step 03 advanced `CP03-4` and preserved desktop startup-evidence built-in instance ownership/default flags through the shared kernel/runtime contract.
- `Kernel Center` now shows persisted startup-evidence `isBuiltIn` and `isDefault` facts in addition to the previously published startup summary fields.
...
- The loop repaired the next missing `CP03-4` shell-facing evidence link:
  - the raw desktop startup-evidence document already contained built-in instance `isBuiltIn` and `isDefault`
  - the desktop kernel publication summary dropped those two boolean fields before shell consumption
  - `Kernel Center` therefore lost whether the startup-time validated instance was the built-in/default projection even though the producer already persisted those facts
- Implemented the narrow repairs:
  - added `builtInInstanceIsBuiltIn` and `builtInInstanceIsDefault` to the desktop kernel startup-evidence summary contract
...
- `docs/review/step-03-kernel-center-startup-evidence-built-in-default-flags-convergence-2026-04-08.md`
- `docs/架构/128-2026-04-08-desktop-startup-evidence-built-in-default-flags-owner.md`
- `docs/review/step-03-执行卡-2026-04-07.md`
- `docs/release/release-2026-04-08-43.md`
...
- The shell now depends on the published built-in startup ownership/default fields remaining aligned with the raw desktop startup-evidence producer.
