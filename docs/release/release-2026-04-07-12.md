...
- 修复 `@sdkwork/claw-install` package root 导出漂移，恢复 Guided Install 组件的显式公共面并重新通过 install contract。
...
- `packages/sdkwork-claw-install/src/index.ts`
- `docs/review/step-03-*`
...
- `node --input-type=module -e "import('./scripts/run-node-typescript-check.mjs').then(({ runNodeTypeScriptChecks }) => runNodeTypeScriptChecks(['packages/sdkwork-claw-install/src/services/openClawBootstrapService.test.ts']))"`
