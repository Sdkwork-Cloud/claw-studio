...
- Step 03 advanced `CP03-4` and preserved desktop startup-evidence descriptor endpoint ID/active port through the shared kernel/runtime contract.
- `Kernel Center` now shows persisted startup descriptor endpoint ID and active port in addition to the previously published startup summary fields.
...
- The loop repaired the next missing `CP03-4` shell-facing evidence link:
  - the raw desktop startup-evidence document already contained descriptor `endpointId` and `activePort`
  - the desktop kernel publication summary dropped those two descriptor endpoint fields before shell consumption
  - `Kernel Center` therefore lost which startup endpoint and active port the snapshot represented, even though the producer already persisted those facts
- Implemented the narrow repairs:
  - added `descriptorEndpointId` and `descriptorActivePort` to the desktop kernel startup-evidence summary contract
...
- `docs/release/release-2026-04-08-46.md`
...
- The shell now depends on the published startup descriptor endpoint fields remaining aligned with the raw desktop startup-evidence producer.
