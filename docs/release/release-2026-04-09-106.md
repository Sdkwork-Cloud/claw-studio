# Claw Studio release-2026-04-09-106
...
- Implemented the Step 07 extraction:
  - added `packages/sdkwork-claw-instances/src/services/openClawProviderPresentation.ts`
  - updated `packages/sdkwork-claw-instances/src/services/index.ts`
  - rewired `packages/sdkwork-claw-instances/src/pages/InstanceDetail.tsx` so the page now gets provider create/edit dialog draft state from the shared helper instead of using raw provider draft factories inline
  - added `packages/sdkwork-claw-instances/src/services/openClawProviderPresentation.test.ts`
