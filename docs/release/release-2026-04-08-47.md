...
- Step 03 advanced `CP03-4` and preserved desktop startup-evidence descriptor requested port and dynamic-port truth through the shared kernel/runtime contract.
- `Kernel Center` now shows the persisted startup descriptor requested port and dynamic-port flag in addition to the previously published startup summary fields.
...
- The loop repaired the next missing `CP03-4` shell-facing evidence link:
  - the raw desktop startup-evidence document already contained descriptor `requestedPort` and `dynamicPort`
  - the desktop kernel publication summary dropped those two descriptor port-allocation fields before shell consumption
  - `Kernel Center` therefore lost what port the startup snapshot had requested and whether it resolved through a dynamic fallback, even though the producer already persisted those facts
- Implemented the narrow repairs:
  - added `descriptorRequestedPort` and `descriptorDynamicPort` to the desktop kernel startup-evidence summary contract
...
- `docs/review/step-03-kernel-center-startup-evidence-descriptor-requested-port-dynamic-port-convergence-2026-04-08.md`
- `docs/架构/132-2026-04-08-desktop-startup-evidence-descriptor-requested-port-owner.md`
- `docs/review/step-03-执行卡-2026-04-07.md`
- `docs/release/release-2026-04-08-47.md`
...
- The shell now depends on the published startup descriptor requested-port and dynamic-port fields remaining aligned with the raw desktop startup-evidence producer.
