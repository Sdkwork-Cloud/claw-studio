...
`@sdkwork/claw-core` 承载跨功能共享的状态和编排能力，例如全局 store 与共享 hooks，但不应该成为功能私有服务的堆放处。

### Types 与 Infrastructure
...
当前工作区是从 `upgrade/claw-studio-v5` 迁移而来，后者仍然是功能、视觉和交互的参考基线。新的分包架构在保持产品一致性的同时，显著提升了后续维护和演进的可控性。
