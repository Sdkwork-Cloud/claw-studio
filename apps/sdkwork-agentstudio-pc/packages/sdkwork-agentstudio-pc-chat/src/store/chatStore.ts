import { createSimpleStore } from '@sdkwork/agentstudio-pc-core';
...
import { STABLE_BUILT_IN_OPENCLAW_INSTANCE_ID as BUILT_IN_DIRECT_CONVERSATION_INSTANCE_ID } from '@sdkwork/agentstudio-pc-types';
import {
...
function createSessionId(instanceId?: string) {
  if (instanceId) {
    return `thread:agent-studio:${uuid()}`;
  }
...
export const openClawGatewaySessions = new OpenClawGatewaySessionStore({
  ...
  createSessionKey(_instanceId, agentId) {
    return buildOpenClawThreadSessionKey(agentId, `agent-studio:${createId('session')}`);
  },
