import { delay } from './serviceTypes.ts';
...
export interface IClawChatService {
  sendMessage(providerId: string, message: string): Promise<ClawChatMessage>;
  getInitialMessages(providerId: string, welcomeText: string): Promise<ClawChatMessage[]>;
}
