use crate::{
  framework::{dialog::{apply_dialog_options, SaveFileOptions}, services::dialog::DialogService},
  state::AppState,
};
...
mod tests {
  ...
  fn writes_blob_bytes_to_selected_path() {
    ...
    DialogService::new().write_blob_to_path(&target, &content).expect("write blob");

    assert_eq!(std::fs::read(&target).expect("read blob"), content);
  }
}

fn create_save_dialog<R: Runtime>(app: &tauri::AppHandle<R>, filename: &str, options: &SaveFileOptions) -> FileDialogBuilder<R> {
  ...
  if let Some(resolved_file_name) = DialogService::new().resolve_default_file_name(filename, options.default_path.as_deref()) {
...
pub async fn save_blob_file(
  ...
  state.context.services.dialog.write_blob_to_path(&target_path, &content)
}
