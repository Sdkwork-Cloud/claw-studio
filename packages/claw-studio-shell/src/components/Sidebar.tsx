...
import { useAppStore, useInstanceStore } from '@sdkwork/claw-studio-business';
      ...
      <nav className={`flex-1 space-y-6 overflow-y-auto overflow-x-hidden scrollbar-hide mt-4 ${isSidebarCollapsed ? 'px-2' : 'px-4'}`}>
                  ...
                  {({ isActive }) => (
                    <>
                      <div className="flex items-center gap-3">
                        ...
                        {!isSidebarCollapsed ? <span className="text-[14px] font-medium tracking-tight">{item.label}</span> : null}
                      </div>
        ...
        <div className="p-4 border-t border-zinc-900 flex flex-col gap-1">
          <NavLink
            ...
            className={({ isActive }) => `flex items-center ${isSidebarCollapsed ? 'justify-center w-10 h-10 mx-auto' : 'px-3 py-2.5 gap-3'} rounded-xl transition-all duration-200 group ${isActive ? 'bg-zinc-800/80 text-white shadow-sm ring-1 ring-white/10' : 'hover:bg-zinc-800/40 hover:text-white text-zinc-400'}`}
          >
            {({ isActive }) => (
              <>
                ...
                {!isSidebarCollapsed ? <span className="text-[14px] font-medium tracking-tight">{t('sidebar.documentation', 'Documentation')}</span> : null}
              </>
            )}
          </NavLink>
          <NavLink
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {!isSidebarCollapsed ? <span className="flex-1 text-[14px] font-medium tracking-tight">{t('sidebar.settings', 'Settings')}</span> : null}
                {!isSidebarCollapsed ? (
