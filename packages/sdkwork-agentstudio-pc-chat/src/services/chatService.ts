import { llmStore } from '@sdkwork/agentstudio-pc-core';
import { instanceStore } from '@sdkwork/agentstudio-pc-core';
import type { Agent, Skill, StudioInstanceRecord } from '@sdkwork/agentstudio-pc-types';
...
const DEFAULT_SYSTEM_INSTRUCTION =
  'You are Agent Studio AI assistant. You help users manage devices, write automation scripts, and answer questions about the ClawHub ecosystem. Keep your answers concise and helpful.';
...
class ChatService implements IChatService {
  ...
  async *sendMessageStream(
    ...
    if (
      ...
      yield `\n\n**${activeInstance.name}** uses the native gateway WebSocket flow. Agent Studio now drives that route through the chat session store instead of the generic HTTP stream service.`;
      return;
    }
    ...
    if (route.mode === 'instanceWebSocket') {
      yield `\n\n**${activeInstance.name}** currently only publishes a WebSocket chat route (${route.websocketUrl}). Configure an HTTP endpoint to enable Agent Studio chat streaming.`;
      return;
    }
