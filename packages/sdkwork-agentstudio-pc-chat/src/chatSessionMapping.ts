...
const AGENT_STUDIO_SESSION_IS_DRAFT_KEY = '__agentstudioIsDraft';
const AGENT_STUDIO_SESSION_TITLE_SOURCE_KEY = '__agentstudioTitleSource';
const AGENT_STUDIO_MESSAGE_TRANSPORT_TEXT_KEY = '__agentstudioTransportText';
const AGENT_STUDIO_MESSAGE_PENDING_DELIVERY_KEY = '__agentstudioPendingDelivery';
...
function buildPersistedSessionNativeMetadata(session: ChatSession) {
  ...
  const metadata = {
    ...
    ...(typeof session.isDraft === 'boolean'
      ? { [AGENT_STUDIO_SESSION_IS_DRAFT_KEY]: session.isDraft }
      : {}),
    ...(titleSource ? { [AGENT_STUDIO_SESSION_TITLE_SOURCE_KEY]: titleSource } : {}),
  };
...
function buildPersistedKernelMessage(params: {
  ...
  return {
    ...
    nativeMetadata: {
      ...
      ...(transportText
        ? {
            [AGENT_STUDIO_MESSAGE_TRANSPORT_TEXT_KEY]: transportText,
          }
        : {}),
      ...(pendingDelivery
        ? {
            [AGENT_STUDIO_MESSAGE_PENDING_DELIVERY_KEY]: true,
          }
        : {}),
    },
  };
...
export function mapStudioMessage(message: StudioConversationMessage): Message | null {
  ...
  const mappedMessage: Message = {
    ...
    transportText:
      readNativeMetadataString(nativeMetadata, AGENT_STUDIO_MESSAGE_TRANSPORT_TEXT_KEY) ??
      undefined,
    ...
    pendingDelivery:
      readNativeMetadataBoolean(nativeMetadata, AGENT_STUDIO_MESSAGE_PENDING_DELIVERY_KEY) === true
        ? true
        : undefined,
    kernelMessage: message.kernelMessage ?? null,
  };
...
export function mapStudioConversation(record: StudioConversationRecord): ChatSession {
  ...
  const restoredDraftState =
    readNativeMetadataBoolean(sessionNativeMetadata, AGENT_STUDIO_SESSION_IS_DRAFT_KEY) ??
  ...
  return projectPersistableChatSessionTitle({
    ...
    titleSource: normalizeChatSessionTitleSource(
      sessionNativeMetadata?.[AGENT_STUDIO_SESSION_TITLE_SOURCE_KEY],
    ),
      ...
      ? {
          ...
          nativeMetadata: {
            ...
            ...(trimOptionalString(message.transportText)
              ? {
                  [AGENT_STUDIO_MESSAGE_TRANSPORT_TEXT_KEY]: trimOptionalString(
