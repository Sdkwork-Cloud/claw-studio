          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
            {topCharts.map((app) => (
              <div 
                ...
                className="flex items-center gap-4 p-3 rounded-2xl hover:bg-white dark:hover:bg-zinc-900 hover:shadow-md dark:hover:shadow-primary-900/10 border border-transparent hover:border-zinc-200 dark:hover:border-zinc-800 transition-all cursor-pointer group"
              >
                <span className="text-lg font-bold text-zinc-400 dark:text-zinc-500 w-6 text-center">{app.rank}</span>
                <img src={app.icon} alt={app.name} className="w-16 h-16 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 object-cover" referrerPolicy="no-referrer" />
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-zinc-900 dark:text-zinc-100 truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{app.name}</h4>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate mb-1">{app.category}</p>
                  <div className="flex items-center gap-1 text-[11px] font-medium text-zinc-500 dark:text-zinc-400">
                    <Star className="w-3 h-3 fill-zinc-400 dark:fill-zinc-500 text-zinc-400 dark:text-zinc-500" />
                    {app.rating}
                  </div>
                </div>
                <button className="bg-zinc-100 dark:bg-zinc-800 text-primary-600 dark:text-primary-400 hover:bg-primary-600 dark:hover:bg-primary-500 hover:text-white dark:hover:text-zinc-900 px-4 py-1.5 rounded-full text-xs font-bold transition-colors">
                  GET
                </button>
              </div>
            ))}
                ...
                >
                  <div className="relative mb-3">
                    <img 
                      src={app.icon} 
                      alt={app.name} 
                      className="w-[140px] h-[140px] rounded-[1.5rem] shadow-sm border border-zinc-200 dark:border-zinc-800 transition-all duration-300 group-hover:scale-105 group-hover:shadow-xl dark:group-hover:shadow-primary-900/20 object-cover" 
                      referrerPolicy="no-referrer"
                    />
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 dark:group-hover:bg-white/5 transition-colors rounded-[1.5rem]" />
                  </div>
                  <h4 className="font-bold text-zinc-900 dark:text-zinc-100 truncate text-sm">{app.name}</h4>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate mb-1.5">{app.category}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 text-xs font-medium text-zinc-500 dark:text-zinc-400">
                      <Star className="w-3.5 h-3.5 fill-zinc-400 dark:fill-zinc-500 text-zinc-400 dark:text-zinc-500" />
                      {app.rating}
                    </div>
                    <button className="bg-zinc-100/80 dark:bg-zinc-800/80 text-primary-600 dark:text-primary-400 hover:bg-primary-600 dark:hover:bg-primary-500 hover:text-white dark:hover:text-zinc-900 px-3 py-1 rounded-full text-[11px] font-bold transition-colors">
                      GET
                    </button>
                  </div>
                </div>
