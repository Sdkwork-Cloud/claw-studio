...
pub fn bootstrap_desktop_host_runtime(
    ...
    if !config.desktop_host.enabled {
        logger.info("desktop embedded host is disabled by config")?;
        return Ok(None);
    }
...
mod tests {
    ...
    fn extract_meta_content(html: &str, name: &str) -> Option<String> {
        let needle = format!("name=\"{name}\" content=\"");
        let start = html.find(&needle)? + needle.len();
        let relative_end = html[start..].find('"')?;
        Some(html[start..start + relative_end].to_string())
    }
    ...
    async fn fetch_browser_session_token(
        ...
        let root_html = client
            .get(browser_base_url)
            .send()
            .await
            .expect("root html response")
            .text()
            .await
            .expect("root html body");

        extract_meta_content(&root_html, "sdkwork-claw-browser-session-token")
            .expect("desktop root html should expose the browser session token")
    }
}
