...
use super::{components::ComponentRegistryService, supervisor::{ManagedServiceLifecycle, SupervisorService}};
...
mod tests {
    ...
    fn component_host_catalog_exposes_openclaw_endpoints_and_docs() {
        ...
        assert_eq!(
            openclaw.service_ids,
            vec!["openclaw_gateway".to_string()]
        );
        assert!(openclaw
            .docs
            .iter()
            .any(|doc| doc.label == "CLI entry" && doc.location.contains("openclaw.mjs")));
    }
    ...
    #[test]
    fn component_host_catalog_marks_openclaw_running_when_supervisor_reports_gateway_running() {
        let root = tempfile::tempdir().expect("temp dir");
        let paths = resolve_paths_for_root(root.path()).expect("paths");
        let supervisor = SupervisorService::new();

        supervisor
            .record_running("openclaw_gateway", Some(45))
            .expect("openclaw gateway running");
        ...
        assert_eq!(openclaw.runtime_status, "running");
    }
}
