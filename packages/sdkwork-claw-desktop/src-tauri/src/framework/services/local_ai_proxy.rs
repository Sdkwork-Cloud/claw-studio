...
impl OpenAiTranslatedStreamState {
    ...
    fn merge_usage(&mut self, usage: &LocalAiProxyTokenUsage) {
        ...
        let merged_total = usage.total_tokens.max(
            self.usage
                .input_tokens
                .saturating_add(self.usage.output_tokens)
                .saturating_add(self.usage.cache_tokens),
        );
...
fn build_openai_response_completed_event(id: &str, model: &str, text: &str) -> Value {
    json!({
        "type": "response.completed",
        "response": build_openai_response_object_from_stream(id, model, text)
    })
}
