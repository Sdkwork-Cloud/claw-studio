...
impl KernelAuthorityState {
    pub fn for_runtime(runtime_id: &str) -> Self {
        Self {
...
pub fn initialize_machine_state(paths: &AppPaths) -> Result<()> {
    let openclaw_kernel_paths = paths.kernel_paths("openclaw");
    ...
    write_json_if_missing(
        &openclaw_kernel_paths.authority_file,
        &KernelAuthorityState::for_runtime("openclaw"),
    )?;
    write_json_if_missing(
        &openclaw_kernel_paths.migrations_file,
        &KernelMigrationState::for_runtime("openclaw"),
    )?;
    write_json_if_missing(&openclaw_kernel_paths.upgrades_file, &RuntimeUpgradesState::default())?;
    Ok(())
}
...
mod tests {
    ...
    fn runtime_upgrade_state_is_not_seeded_by_openclaw_default_only() {
        let state = RuntimeUpgradesState::default();
    ...
    fn kernel_authority_state_defaults_are_kernel_neutral() {
        ...
        let migrations = KernelMigrationState::default();

        assert!(state.runtime_id.is_empty());
        assert!(migrations.runtime_id.is_empty());
    }
