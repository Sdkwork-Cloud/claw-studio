...
export function InstanceFilesWorkspace(props: InstanceFilesWorkspaceProps) {
  ...
  const contextAgent =
    props.mode === 'instance'
      ? props.agents.find((agent) => agent.agent.id === props.selectedAgentId) || null
      : props.agent;
  const visibleFiles =
    props.mode === 'instance' ? getAgentScopedWorkbenchFiles(props.files, contextAgent) : props.files;
  const scopeKey =
    props.mode === 'instance'
      ? props.selectedAgentId || '__instance-files__'
