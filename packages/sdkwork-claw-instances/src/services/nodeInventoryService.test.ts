...
await runTest('nodeInventoryService separates the local managed kernel node from attached remote nodes', async () => {
  const { createNodeInventoryService } = await import('./nodeInventoryServiceCore.ts');
...
await runTest('nodeInventoryService maps degraded kernel health and metadata-only remote instances as attached nodes', async () => {
  const { createNodeInventoryService } = await import('./nodeInventoryServiceCore.ts');
