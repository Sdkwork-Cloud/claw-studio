...
export function ApiRouter() {
  ...
  const signInMutation = useMutation({
    ...
    onSuccess: async (session) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['api-router'] });
    },
    onError: (error) => {
      toast.error(
  ...
  const signOutMutation = useMutation({
    ...
    onError: () => {
      toast.error(t('apiRouterPage.admin.toast.disconnectFailed'));
    },
  });

  const adminStatusCard = adminStatus ? (
    <ApiRouterAdminStatusCard
      status={adminStatus}
      ...
      onLogin={(input) => signInMutation.mutate(input)}
  ...
  return (
    <div data-slot="api-router-page" className="h-full overflow-y-auto bg-zinc-50 dark:bg-zinc-950">
      <div className="flex w-full flex-col gap-4 px-4 py-4 sm:px-4 sm:py-6 xl:px-4 xl:py-6">
        {adminStatusCard}
        {runtimeStatus ? <ApiRouterRuntimeStatusCard status={runtimeStatus} /> : null}
