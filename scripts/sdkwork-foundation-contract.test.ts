...
runTest('sdkwork-agentstudio-pc-i18n is implemented locally instead of re-exporting agent-studio infrastructure', () => {
  const pkg = readJson<{ dependencies?: Record<string, string> }>('packages/sdkwork-agentstudio-pc-i18n/package.json');
  const source = read('packages/sdkwork-agentstudio-pc-i18n/src/index.ts');

  assert.ok(exists('packages/sdkwork-agentstudio-pc-i18n/src/locales/en.json'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-i18n/src/locales/zh.json'));
  assert.ok(!pkg.dependencies?.['@sdkwork/agentstudio-pc-studio-infrastructure']);
  assert.doesNotMatch(source, /@sdkwork\/agent-studio-infrastructure/);
  assert.match(source, /ensureI18n/);
});

runTest('sdkwork-agentstudio-pc-types is implemented locally instead of re-exporting agent-studio domain', () => {
  const pkg = readJson<{ dependencies?: Record<string, string> }>('packages/sdkwork-agentstudio-pc-types/package.json');
  const source = read('packages/sdkwork-agentstudio-pc-types/src/index.ts');

  assert.ok(exists('packages/sdkwork-agentstudio-pc-types/src/service.ts'));
  assert.ok(!pkg.dependencies?.['@sdkwork/agentstudio-pc-studio-domain']);
  assert.doesNotMatch(source, /@sdkwork\/agent-studio-domain/);
  assert.match(source, /export \* from '.\/service(?:\.ts)?'/);
});

runTest('sdkwork-agentstudio-pc-distribution is implemented locally instead of re-exporting agent-studio distribution', () => {
  const pkg = readJson<{ dependencies?: Record<string, string> }>('packages/sdkwork-agentstudio-pc-distribution/package.json');
  const source = read('packages/sdkwork-agentstudio-pc-distribution/src/index.ts');

  assert.ok(exists('packages/sdkwork-agentstudio-pc-distribution/src/manifests/cn/index.ts'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-distribution/src/manifests/global/index.ts'));
  assert.ok(!pkg.dependencies?.['@sdkwork/agentstudio-pc-studio-distribution']);
  assert.doesNotMatch(source, /@sdkwork\/agent-studio-distribution/);
  assert.match(source, /getDistributionManifest/);
});

runTest('sdkwork-agentstudio-pc-web pins stable build chunks for infrastructure without shipping mock-only chunks', () => {
  const viteConfigSource = read('packages/sdkwork-agentstudio-pc-web/vite.config.ts');
  const buildHelperSource = read('scripts/viteBuildOptimization.ts');
  const registrySource = read('packages/sdkwork-agentstudio-pc-infrastructure/src/platform/registry.ts');
  ...
  assert.match(buildHelperSource, /sdkwork-agentstudio-pc-infrastructure/);
  ...
  assert.doesNotMatch(viteConfigSource, /agent-studio-mock/);
  assert.doesNotMatch(viteConfigSource, /sdkwork-agentstudio-pc-community\/src\/NewPost\.tsx/);
  assert.doesNotMatch(viteConfigSource, /sdkwork-agentstudio-pc-community\/src\/pages\/community\/NewPost\.tsx/);
  assert.doesNotMatch(viteConfigSource, /sdkwork-agentstudio-pc-community\/src\/CommunityPostDetail\.tsx/);
  assert.doesNotMatch(
    viteConfigSource,
    /sdkwork-agentstudio-pc-community\/src\/pages\/community\/CommunityPostDetail\.tsx/,
  );
  assert.doesNotMatch(viteConfigSource, /sdkwork-agentstudio-pc-chat\/src\/components\/ChatMessage\.tsx/);
  assert.doesNotMatch(viteConfigSource, /@sdkwork\/agent-studio-infrastructure/);
...
runTest('infrastructure root keeps mock-service helpers private instead of exporting them to runtime consumers', () => {
  const infrastructureIndexSource = read('packages/sdkwork-agentstudio-pc-infrastructure/src/index.ts');
  const servicesIndexSource = read('packages/sdkwork-agentstudio-pc-infrastructure/src/services/index.ts');
...
runTest('mock studio helpers are removed from the production source tree', () => {
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/studioMockService.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/studioMockServiceProxy.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/studioMockService.test.ts'));
});

runTest('dead raw-http business helper files are removed after app-sdk migration', () => {
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/http/apiClient.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/authClient.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/authClient.test.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/userClient.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/accountClient.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/services/notificationClient.ts'));
});
...
runTest('foundation removes legacy api-router runtime bridge files and dead locale blocks after extraction', () => {
  ...
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/platform/webApiRouter.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/platform/webApiRouter.test.ts'));
  assert.ok(!exists('packages/sdkwork-agentstudio-pc-infrastructure/src/platform/contracts/apiRouter.ts'));
  ...
  assert.doesNotMatch(pnpmLockSource, /packages\/sdkwork-agentstudio-pc-apirouter:/);
...
runTest('foundation composes local api proxy from sdkwork-local-router instead of appbase or local compat helpers', () => {
  ...
  const settingsPackage = readJson<{ dependencies?: Record<string, string> }>(
    'packages/sdkwork-agentstudio-pc-settings/package.json',
  );
  const desktopCargoSource = read('packages/sdkwork-agentstudio-pc-desktop/src-tauri/Cargo.toml');
  ...
  const providerConfigImportServiceSource = read(
    'packages/sdkwork-agentstudio-pc-settings/src/services/providerConfigImportService.ts',
  );
  const providerConfigEditorPolicySource = read(
    'packages/sdkwork-agentstudio-pc-settings/src/services/providerConfigEditorPolicy.ts',
  );
  const localAiProxyLogsServiceSource = read(
    'packages/sdkwork-agentstudio-pc-settings/src/services/localAiProxyLogsService.ts',
  );
  ...
  const localAiProxySnapshotSource = read(
    'packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy_snapshot.rs',
  );
  const localAiProxyObservabilitySource = read(
    'packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy_observability.rs',
  );
  ...
  assert.equal(exists('packages/sdkwork-agentstudio-pc-core/src/services/json5Compat.ts'), false);
  assert.equal(exists('packages/sdkwork-agentstudio-pc-core/src/services/legacyProviderCompat.ts'), false);
  assert.equal(exists('packages/sdkwork-agentstudio-pc-core/src/services/localAiProxyRouteService.ts'), false);
});
...
runTest('foundation sources Hermes kernel-config path semantics from the shared local-api-proxy package boundary', () => {
  const hermesKernelConfigPathServiceSource = read(
    'packages/sdkwork-agentstudio-pc-core/src/services/hermesKernelConfigPathService.ts',
  );
  const hermesPathResolutionServiceSource = read(
    'packages/sdkwork-agentstudio-pc-core/src/services/hermesPathResolutionService.ts',
  );
...
runTest('foundation sources local ai proxy generic support, upstream, and probe helpers from the shared native package boundary', () => {
  ...
  assert.equal(
    exists('packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy/support.rs'),
    false,
  );
  assert.equal(
    exists('packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy/upstream.rs'),
    false,
  );
  assert.equal(
    exists('packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy/probe.rs'),
    false,
  );
});
...
runTest('foundation keeps obsolete gateway docs and implementation plans out of the workspace docs surface', () => {
  const matches = findFilesContaining(
    'docs',
    /API Router|api router|@sdkwork\/claw-apirouter|sdkwork-agentstudio-pc-apirouter|api-router-|apirouter|apiRouter|ApiRouter|VITE_API_ROUTER_|openApiRouter/,
  );
