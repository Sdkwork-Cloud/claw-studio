...
runTest('sdkwork-claw-huggingface keeps the V5 huggingface package surface locally', () => {
  ...
  assert.ok(
    exists('packages/sdkwork-claw-huggingface/src/pages/huggingface/HuggingFaceModelDetail.tsx'),
  );
...
runTest('sdkwork-claw-huggingface preserves the V5 discovery shell and visual badge', () => {
  ...
  assert.match(pageSource, /t\('huggingface\.models\.title'\)/);
});

runTest('sdkwork-claw-huggingface preserves the V5 model detail tabs and visual fallback', () => {
  const detailSource = read(
    'packages/sdkwork-claw-huggingface/src/pages/huggingface/HuggingFaceModelDetail.tsx',
  );

  assert.match(detailSource, /useTranslation/);
  assert.match(detailSource, /'model_card' \| 'files' \| 'community'/);
  assert.match(detailSource, /t\('huggingface\.detail\.downloadToLocal'\)/);
  assert.match(detailSource, /t\('huggingface\.detail\.stats\.title'\)/);
});
