...
export function inspectContainerDeploymentContract({
  ...
  if (defaultProfileEntries.get('CLAW_SERVER_DATA_DIR') !== '/var/lib/agentstudio-server') {
    throw new Error(
      `Packaged container runtime profile must pin CLAW_SERVER_DATA_DIR=/var/lib/agentstudio-server in ${defaultProfilePath}.`,
    );
  }
  ...
  if (!/^\s*-\s+[^:\r\n]+:\/var\/lib\/agentstudio-server\s*$/m.test(composeDefinition)) {
    throw new Error(
      `Packaged docker compose must persist /var/lib/agentstudio-server in ${composePath}.`,
    );
  }
...
export async function smokeKubernetesDeploymentBundle({
  ...
  if (
    ...
    || !/mountPath:\s*\/var\/lib\/agentstudio-server\b/m.test(renderedManifest)
  ) {
    throw new Error(
      'Rendered kubernetes manifests must mount /var/lib/agentstudio-server through a PersistentVolumeClaim.',
    );
  }
  checks.push({
    id: 'persistent-storage',
